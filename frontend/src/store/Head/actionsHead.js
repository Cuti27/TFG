...
export default {
  async getCabezales({ commit, state }, page = 1) {
    commit("addIsLoading");
    const request =
      "http://josemiguel.myqnapcloud.com:41063/api/head?page=" + page;
    ...
    let response = await axios
      ...
      .catch((err) => {
        if (err.response) {
          ...
          commit(
            "addGlobalError",
            "Error al cargar los cabezales, intentalo más tarde"
          );
          return null;
        }
      });
    ...
    let response = await axios
      ...
      .catch((err) => {
        if (err.response) {
          ...
          commit(
            "addGlobalError",
            "No se ha podido crear el cabezal, intentalo más tarde"
          );
          commit("removeIsLoading");
          return null;
        }
  ...
  async updateHead({ commit, dispatch, state }, data) {
    commit("addIsLoading");
    const request =
      "http://josemiguel.myqnapcloud.com:41063/api/head/" + data.id;
    console.log(state.auth);
    await axios
      .post(request, data, await addAuthHeader(state.auth))
      .catch((err) => {
        if (err.response) {
          ...
          commit(
            "addGlobalError",
            "No se ha podido actualizar el cabezal, intentalo más tarde"
          );
          commit("removeIsLoading");
          return null;
        }
    ...
    let response = await axios
      ...
      .catch((err) => {
        if (err.response) {
          ...
          commit(
            "addGlobalError",
            "No se ha podido borrar el cabezal, intentalo más tarde"
          );
          commit("removeIsLoading");
          return null;
        }
    ...
    let response = await axios
      ...
      .catch((err) => {
        if (err.response) {
          ...
          commit(
            "addGlobalError",
            "Error, no se ha podido obtener los datos de los programas"
          );
          commit("removeIsLoading");
          return null;
        }
  ...
  async fetchAllProgram({ commit, state }) {
    const request = "http://josemiguel.myqnapcloud.com:41063/api/allProgram";
    let response = await axios
      .get(request, await addAuthHeader(state.auth))
      .catch((err) => {
        if (err.response) {
